...
/**
 * Upsert the singleton general session.
 * Uses INSERT … ON CONFLICT DO NOTHING to avoid the race condition where
 * two concurrent requests both see no existing session and both try to insert.
 */
async function createOrFind({ documentId, type, title, messages = [] }) {
  const id = randomUUID();
  const result = await query(
    `
      INSERT INTO chat_sessions (id, document_id, type, title, messages)
      VALUES ($1, $2, $3, $4, $5::jsonb)
      ON CONFLICT (type) WHERE type = 'general' DO NOTHING
      RETURNING *
    `,
    [
      id,
  ...
  return findOne({ documentId, type });
}
